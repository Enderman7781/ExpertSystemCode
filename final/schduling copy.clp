...
(defrule schedule-drop-occupy
        ...
        ?drop <- (lesson (ID ?else)(state 1)(time $?occupy-time)(teacher ?else-teacher))
        (favorite-time (teacher ?else-teacher) (time $?ef-t))
        ?drop-w <- (teacher (ID ?teacher) (weight ?else-weight))
        (favorite-time (teacher ?other-teacher&~?else-teacher)(time $?of-t))

        (test 
            (not                     
                (> 
                        (+ (if (member$ ?t1 $?of-t) then 1 else 0)
                            (if (member$ ?t2 $?of-t) then 1 else 0)
                            (if (member$ ?t3 $?of-t) then 1 else 0)
                        )
                        (+
                            (if (member$ ?t1 $?ef-t) then 1 else 0)
                            (if (member$ ?t2 $?ef-t) then 1 else 0)
                            (if (member$ ?t3 $?ef-t) then 1 else 0)
                        )
                )                
            )
        )

        (test (or (member$ ?t1 $?occupy-time)(member$ ?t2 $?occupy-time)(member$ ?t3 $?occupy-time)))
        ...
        (assert (phase get-lesson))
        ;喜好有三門, +0
        (modify ?f3 (weight (+ ?weight ?add-weight)))
        (printout t "Drop " ?else " weight " ?retract-weight " then Add " ?select " weight " ?add-weight crlf)
        ; 這個時段教室有人了
        (modify ?f2 (state 1) (time ?t1 ?t2 ?t3) (room ?classroom ?classroom ?classroom))
        (modify ?drop (state 0) (time) (room))
        (modify ?drop-w (weight (- ?else-weight ?retract-weight)))
